...
// Self-contained status-feedback helper for the OFM-BusPowerSupply module.
namespace BpsStatus
{
    ...
    //   - Cyclic sending (cyclicTimeMS) is independent of the change detection above.
    ...
    // Binary on/off status. changeSend gates the send-on-change only; cyclic sending (cyclicTimeMS)
    // is independent of changeSend, matching this module's separate "send on change" and
    // "send cyclically" options.
    inline void sendSwitch(GroupObject &ko, bool changeSend, bool stateOn, uint32_t cyclicTimeMS, uint32_t &cyclicTimer)
    {
        if (changeSend && (bool)ko.value(DPT_Switch) != stateOn)
